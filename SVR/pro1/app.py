...
option = st.radio("Choose data source", ["Download Sample Dataset", "Upload CSV"])

if option == "Download Sample Dataset":
    if st.button("Download Dataset"):
        url = "https://raw.githubusercontent.com/selva86/datasets/master/BostonHousing.csv"
        df = pd.read_csv(url)
        path = os.path.join(RAW_DIR, "housing.csv")
...
if st.session_state["df_model"] is not None:
    ...
    target = st.selectbox("Select Target Column (Numeric)", df_model.columns)
    y = df_model[target]

    if y.dtype == "object":
        st.error("SVR requires a numeric (continuous) target")
        st.stop()
